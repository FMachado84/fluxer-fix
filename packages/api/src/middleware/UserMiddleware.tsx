...
export const UserMiddleware = createMiddleware<HonoEnv>(async (ctx, next) => {
	...
	if (type === 'bearer') {
		...
		if (accessToken) {
			// OAuth2 access token — primary bearer path
			recordCounter({
				name: 'auth.token_validation',
		...
		// Intentional fallback: a session token supplied with a "Bearer " prefix is also
		// accepted here for client compatibility. authTokenType is set to 'session' so
		// downstream middleware can distinguish the two paths.
		const authService = ctx.get('authService');
		const authSession = await authService.getAuthSessionByToken(token);
		if (authSession) {
			recordCounter({

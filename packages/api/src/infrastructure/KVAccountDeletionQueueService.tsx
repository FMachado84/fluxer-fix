...
export class KVAccountDeletionQueueService {
	...
	private deserializeQueueItem(value: string): QueuedDeletion | null {
		const separatorIndex = value.indexOf('|');
		if (separatorIndex === -1 || separatorIndex === 0 || separatorIndex === value.length - 1) {
			Logger.warn({value}, 'Deletion queue: malformed item, skipping');
			return null;
		}
		const rawUserId = value.slice(0, separatorIndex);
		const rawReasonCode = value.slice(separatorIndex + 1);
		const deletionReasonCode = parseInt(rawReasonCode, 10);
		if (Number.isNaN(deletionReasonCode)) {
			Logger.warn({value}, 'Deletion queue: invalid reason code, skipping');
			return null;
		}
		try {
			return {userId: BigInt(rawUserId), deletionReasonCode};
		} catch {
			Logger.warn({value}, 'Deletion queue: invalid userId, skipping');
			return null;
		}
	}

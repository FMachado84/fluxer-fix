...
export async function initializeWorkerDependencies(snowflakeService: SnowflakeService): Promise<WorkerDependencies> {
	...
	if (Config.voice.enabled) {
		...
		voiceTopology = new VoiceTopology(voiceRepository, kvClient);
...
export async function shutdownWorkerDependencies(deps: WorkerDependencies): Promise<void> {
	...
	try {
		await deps.snowflakeService.shutdown();
		Logger.info('SnowflakeService shut down');
	} catch (error) {
		Logger.error({error}, 'Error shutting down SnowflakeService');
	}

	try {
		deps.activityTracker.shutdown();
		Logger.info('KVActivityTracker shut down');
	} catch (error) {
		Logger.error({error}, 'Error shutting down KVActivityTracker');
	}

	if (deps.voiceTopology) {
		try {
			deps.voiceTopology.shutdown();
			Logger.info('VoiceTopology shut down');
		} catch (error) {
			Logger.error({error}, 'Error shutting down VoiceTopology');
		}
	}

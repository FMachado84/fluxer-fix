...
const CRON_LOCK_TTL_SECONDS = 65;
...
export class CronScheduler {
	...
	constructor(queue: JetStreamWorkerQueue, logger: LoggerInterface, kvClient: IKVProvider | null = null) {
		this.queue = queue;
		this.logger = logger;
	...
	private async tick(): Promise<void> {
		...
		for (const def of this.definitions.values()) {
			...
			if (matchesCronExpression(def.cronExpression, now)) {
				...
				try {
					if (this.kvClient) {
						const lockKey = `cron:lock:${def.id}:${nowSeconds}`;
						const acquired = await this.kvClient.setnx(lockKey, '1', CRON_LOCK_TTL_SECONDS);
						if (!acquired) {
							this.logger.debug({cronId: def.id}, 'Cron lock not acquired, another replica already fired this job');
							continue;
						}
					}

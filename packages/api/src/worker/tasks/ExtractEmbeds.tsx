...
const extractEmbeds: WorkerTaskHandler = async (payload, helpers) => {
	...
	if (urlsToUnfurl.length === 0 && cachedEmbedsByUrl.size === 0) {
		Logger.debug({messageId}, 'Skipping extractEmbeds: no embeds to process');
		return;
	}

	try {
		const isNSFWAllowed = validated.isNSFWAllowed ?? false;
		const unfurledEmbedsByUrl: Map<string, Array<MessageEmbed>> =
			urlsToUnfurl.length > 0 ? await unfurlUrls(urlsToUnfurl, embedService, isNSFWAllowed) : new Map();

		if (unfurledEmbedsByUrl.size === 0 && cachedEmbedsByUrl.size === 0) {

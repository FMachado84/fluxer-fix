...
async function processItem(
	...
	try {
		...
		try {
			...
			Logger.error(
				{requeueError, s3Key: item.s3Key, cdnUrl: item.cdnUrl, reason: item.reason},
				'CRITICAL: failed to requeue asset deletion item — item will be permanently lost from queue',
			);
		}
		throw error;
	}
